...
mod tests {
	...
	fn should_disable_signer_in_geth_compat() {
		...
		// when
		let conf0 = parse(&["purity", "--geth"]);
		let conf1 = parse(&["purity", "--geth", "--force-ui"]);
		let conf2 = parse(&["purity", "--geth", "ui"]);
		let conf3 = parse(&["purity"]);
	...
	fn should_disable_signer_when_account_is_unlocked() {
		...
		// when
		let conf0 = parse(&["purity", "--unlock", "0x0"]);
	...
	fn should_parse_ui_configuration() {
		...
		let conf5 = parse(&["purity", "--ui-path=signer", "ui"]);
