			...
			Prototype::List(2) => {
				...
				match (is_leaf, partial.common_prefix(&existing_key)) {
					...
					(_, 0) => {
						...
						trace!("no-common-prefix, not-both-empty (exist={:?}; new={:?}): TRANSMUTE,AUGMENT",
						       existing_key.len(),
						       partial.len());
					...
					(_, cp) => {
						...
						trace!("partially-shared-prefix (exist={:?}; new={:?}; cp={:?}): AUGMENT-AT-END",
						       existing_key.len(),
						       partial.len(),
						       cp);
			...
			Node::Branch(nodes, node_value) => {
				...
				match (used_index, node_value) {
					...
					(UsedIndex::One(a), None) => {
						...
						MaybeChanged::Changed(Self::encoded(self.fixed(Node::Extension(NibbleSlice::new_offset(&new_partial[..], 1),
						                                                               nodes[a as usize]),
						                                               journal)))
					}
			...
			Node::Extension(partial, payload) => {
				match Node::decoded(self.get_raw_or_lookup(payload)) {
					Node::Extension(sub_partial, sub_payload) => {
						...
						MaybeChanged::Changed(Self::encoded(Self::fixed_indirection(Node::Extension(NibbleSlice::new_composed(&partial,
						                                                                                                      &sub_partial),
						                                                                            sub_payload),
						                                                            journal)))
					}
					Node::Leaf(sub_partial, sub_value) => {
						// combine with node below
						journal.delete_node(payload);
						MaybeChanged::Changed(Self::encoded(Self::fixed_indirection(Node::Leaf(NibbleSlice::new_composed(&partial, &sub_partial),
						                                                                       sub_value),
						                                                            journal)))
					}
...
mod tests {
	...
	use bytes::{Bytes, Populatable, ToPretty};
