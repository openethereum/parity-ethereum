...
impl SecretStore {
	...
	pub fn collect_garbage(&mut self) {
		...
		let expired_addresses = garbage_lock.iter()
		                                    .filter(|&(_, unlock)| {
			                                    match unlock.expires {
				                                    Some(ref expire_val) => expire_val < &utc,
				                                    _ => false,
			                                    }
			                                   })
...
impl EncryptedHashMap<H128> for SecretStore {
	...
	fn remove<Value: FromRawBytes + BytesConvertable>(&mut self, key: &H128, password: Option<&str>) -> Option<Value> {
		let previous = if let Some(pass) = password { if let Ok(previous_value) = self.get(&key, pass) { Some(previous_value) } else { None } } else { None };
...
mod tests {
	...
	fn pregenerate_keys(temp: &RandomTempPath, count: usize) -> Vec<H128> {
		...
		for _ in 0..count {
			result.push(write_sstore.directory
			                        .save(KeyFileContent::new(KeyFileCrypto::new_pbkdf2(FromHex::from_hex("5318b4d5bcd28de64ee5559e671353e16f075ecae9f99c7a79a38af5f869aa46").unwrap(), H128::from_str("6087dab2f9fdbbfaddc31a909735c1e6").unwrap(), H256::from_str("ae3cd4e7013836a3df6bd7241b12db061dbe2c6785853cce422d148a624ce0bd").unwrap(), H256::from_str("517ead924a9d0dc3124507e3393d175ce3ff7c1e96529c6c555ce9e51205e9b2").unwrap(), 262144, 32)))
			                        .unwrap());
		}
		result
	...
	fn accounts_expire() {
		...
		assert!(svc.account_secret(&address).is_ok());
		{
			...
			if let Entry::Occupied(mut occupied) = entry {
				occupied.get_mut().expires = Some(UTC::now() - Duration::minutes(1))
			}
		}
