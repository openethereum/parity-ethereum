...
impl SocketAddrExt for Ipv6Addr {
	...
	fn is_global_s(&self) -> bool {
		if self.is_multicast() {
			...
			!self.is_loopback() && !((self.segments()[0] & 0xffc0) == 0xfe80) && !((self.segments()[0] & 0xffc0) == 0xfec0) &&
	...
	fn convert_sockaddr(sa: *mut sockaddr) -> Option<IpAddr> {
		...
		let (addr, _) = match unsafe { *sa }.sa_family as i32 {
			AF_INET => {
				...
				(IpAddr::V4(Ipv4Addr::new((addr & 0x000000FF) as u8,
			...
			AF_INET6 => {
				...
				(IpAddr::V6(Ipv6Addr::new(addr[0], addr[1], addr[2], addr[3], addr[4], addr[5], addr[6], addr[7])),
				 port)
			}
			_ => return None,
		};
		...
		match search_gateway_from_timeout(local_addr.ip().clone(), Duration::new(5, 0)) {
			...
			Ok(gateway) => {
				match gateway.get_external_ip() {
					...
					Ok(external_addr) => {
						match gateway.add_any_port(PortMappingProtocol::TCP,
						                           SocketAddrV4::new(local_addr.ip().clone(), local_addr.port()),
						                           0,
						                           "Parity Node/TCP") {
							Err(ref err) => {
							...
							Ok(tcp_port) => {
								match gateway.add_any_port(PortMappingProtocol::UDP,
								                           SocketAddrV4::new(local_addr.ip().clone(), local.udp_port),
								                           0,
								                           "Parity Node/UDP") {
									Err(ref err) => {
										debug!("Port mapping error: {}", err);
