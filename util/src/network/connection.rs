...
impl EncryptedConnection {
	...
	fn read_payload(&mut self, payload: &[u8]) -> Result<Packet, UtilError> {
		...
		self.decoder
		    .decrypt(&mut RefReadBuffer::new(&payload[0..self.payload_len]), &mut RefWriteBuffer::new(&mut packet), false)
		    .expect("Invalid length or padding");
	...
	pub fn readable<Message>(&mut self, io: &IoContext<Message>) -> Result<Option<Packet>, UtilError>
