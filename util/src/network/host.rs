    ...
    where Message: Send + Sync + Clone + 'static,
{
	...
	fn new(io: &'s IoContext<NetworkIoMessage<Message>>, protocol: ProtocolId, session: Option<SharedSession>, sessions: Arc<RwLock<Slab<SharedSession>>>) -> NetworkContext<'s, Message> {
    ...
    where Message: Send + Sync + Clone,
{
	...
	fn init_public_interface(&self, io: &IoContext<NetworkIoMessage<Message>>) -> Result<(), UtilError> {
		...
		let discovery = {
			...
			if info.config.discovery_enabled && !info.config.pin { Some(Discovery::new(&info.keys, public_endpoint.address.clone(), public_endpoint, DISCOVERY)) } else { None }
		};
