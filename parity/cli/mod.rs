...
usage! {
	...
	}
	{
		...
		["Footprint Options"]
			...
			ARG arg_pruning_historical_eras: (String) = "", or |c: &Config| c.footprint.as_ref()?.pruning_historical_eras.as_ref().map(|vec| vec.iter().map(|&(h, l)| vec![h.to_string(), l.to_string()].join("-")).collect::<Vec<_>>().join(",")),
			"--pruning-historical-eras=[BLOCK_RANGES]",
			"Sets inclusive block ranges for which we should retain state.",
...
struct Footprint {
	...
	pruning_historical_eras: Option<Vec<(u64, u64)>>,
