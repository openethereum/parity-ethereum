...
#[cfg(test)]
mod tests {
	use super::{Args, ArgsError, Config, Operating, Account, Ui, Network, Ws, Rpc, Ipc, Dapps, Ipfs, Mining, Footprint, Snapshots, VM, Misc, SecretStore};
	use toml;
	...
	fn should_deserialize_toml_file() {
		...
		assert_eq!(config,
		           Config {
		               ...
		               snapshots: Some(Snapshots { disable_periodic: Some(true) }),
		               vm: Some(VM { jit: Some(false) }),
		               misc: Some(Misc {
