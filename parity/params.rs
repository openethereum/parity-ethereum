...
impl SpecType {
	pub fn spec<'a, T: Into<SpecParams<'a>>>(&self, params: T) -> Result<Spec, String> {
		...
		match *self {
			...
			SpecType::Stureby => Ok(ethereum::new_stureby(params)),
...
mod tests {
	...
	fn test_spec_type_parsing() {
		...
		assert_eq!(SpecType::Stureby, "stureby".parse().unwrap());
	}
	...
	fn test_spec_type_display() {
		...
		assert_eq!(format!("{}", SpecType::Stureby), "stureby");
