...
/**	/// When initialization request is received.
	...
	pub fn on_share_remove_request(&self, sender: &NodeId, message: &ShareRemoveRequest) -> Result<(), Error> {
		...
		// only process if we are waiting for this request
		{
			...
			let all_nodes_set = self.core.key_share.id_numbers.keys().filter(|n| !shares_to_remove.contains(n)).collect::<Vec<_>>();
...
println!("=== zzz");
		...
		let shares_to_remove = data.shares_to_remove.as_ref().expect("TODO");
		for share_source in shares_to_remove.iter().filter(|n| **n != core.meta.self_node_id) {
			core.transport.send(share_source, ShareRemoveMessage::ShareRemoveRequest(ShareRemoveRequest {
...
println!("=== {} -> {}: {}", from, to, message);
	...
	fn node_removed_using_share_remove() {
		...
		ml.nodes[&master].session.initialize(nodes_to_remove.clone(), Some(old_set_signature), Some(new_set_signature));
		ml.run();
