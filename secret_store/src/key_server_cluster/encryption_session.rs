...
impl Session {
	...
	pub fn on_initialize_session(&self, sender: NodeId, mut message: InitializeSession) -> Result<(), Error> {
		...
		let mut derived_point = message.derived_point.into();
		...
		match next_receiver {
			Some(next_receiver) => {
				return self.cluster.send(&next_receiver, Message::InitializeSession(InitializeSession {
						session: self.id.clone().into(),
						derived_point: message.derived_point.into(),
					}));
			},
			None => {
	...
	pub fn on_complete_initialization(&self, sender: NodeId, message: CompleteInitialization) -> Result<(), Error> {
		...
		data.derived_point = Some(message.derived_point.into());
		data.nodes = message.nodes.into_iter().map(|(id, number)| (id.into(), NodeData::with_id_number(number.into()))).collect();
	...
	pub fn on_keys_dissemination(&self, sender: NodeId, message: KeysDissemination) -> Result<(), Error> {
		...
		// update node data
		{
			...
			node_data.secret1 = Some(message.secret1.into());
			node_data.secret2 = Some(message.secret2.into());
			node_data.publics = Some(message.publics.into_iter().map(Into::into).collect());
		}
	...
	pub fn on_public_key_share(&self, sender: NodeId, message: PublicKeyShare) -> Result<(), Error> {
		...
		// update node data with received public share
		{
			...
			node_data.public_share = Some(message.public_share.into());
		}
