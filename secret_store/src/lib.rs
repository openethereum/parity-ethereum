...
pub fn start(client: Arc<Client>, sync: Arc<SyncProvider>, self_key_pair: Arc<NodeKeyPair>, config: ServiceConfiguration) -> Result<Box<KeyServer>, Error> {
	...
	let key_server_set = key_server_set::OnChainKeyServerSet::new(&client, &sync, self_key_pair.clone(), config.cluster_config.nodes.clone())?; // TODO: return empty set until fully synced
