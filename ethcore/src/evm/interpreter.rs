...
impl evm::Evm for Interpreter {
	fn exec(&self, params: ActionParams, ext: &mut evm::Ext) -> evm::Result {
		...
		while reader.position < code.len() {
			...
			evm_debug!({
				println!("[0x{:x}][{}(0x{:x}) Gas: {:x}\n  Gas Before: {:x}", reader.position, color(instruction, instructions::get_info(instruction).name), instruction, gas_cost, current_gas + gas_cost);
			});
...
impl Interpreter {
	...
	fn get_gas_cost_mem(&self, ext: &evm::Ext, instruction: Instruction, mem: &mut Memory, stack: &Stack<U256>) -> Result<(U256, usize), evm::Error> {
		...
		let cost = match instruction {
			...
			instructions::CALL | instructions::CALLCODE => {
				...
				let mem = cmp::max(try!(self.mem_needed(stack.peek(5), stack.peek(6))), try!(self.mem_needed(stack.peek(3), stack.peek(4))));
			...
			instructions::DELEGATECALL => {
				...
				let mem = cmp::max(try!(self.mem_needed(stack.peek(4), stack.peek(5))), try!(self.mem_needed(stack.peek(2), stack.peek(3))));
				InstructionCost::GasMem(gas, mem)
			}
			instructions::CREATE => {
	...
	fn mem_gas_cost(&self, schedule: &evm::Schedule, current_mem_size: usize, mem_size: &U256) -> Result<(U256, usize), evm::Error> {
		let gas_for_mem = |mem_size: U256| {
			...
			if b > U512::from(!U256::zero()) { Err(evm::Error::OutOfGas) } else { Ok(overflowing!(a.overflowing_add(U256::from(b)))) }
		};
		...
		Ok((if req_mem_size_rounded > current_mem_size { new_mem_gas - current_mem_gas } else { U256::zero() }, req_mem_size_rounded.low_u64() as usize))
	}
	...
	fn exec_instruction(&self, gas: Gas, params: &ActionParams, ext: &mut evm::Ext, instruction: Instruction, code: &mut CodeReader, mem: &mut Memory, stack: &mut Stack<U256>) -> Result<InstructionResult, evm::Error> {
		match instruction {
			instructions::JUMP => {
	...
	fn copy_data_to_memory(&self, mem: &mut Memory, stack: &mut Stack<U256>, data: &[u8]) {
		...
		let output_end = match source_offset > source_size || size > source_size || source_offset + size > source_size {
			true => {
				let zero_slice = if source_offset > source_size { mem.writeable_slice(dest_offset, size) } else { mem.writeable_slice(dest_offset + source_size - source_offset, source_offset + size - source_size) };
	...
	fn verify_instructions_requirements(&self, info: &instructions::InstructionInfo, stack_limit: usize, stack: &Stack<U256>) -> Result<(), evm::Error> {
	...
	fn verify_jump(&self, jump_u: U256, valid_jump_destinations: &HashSet<usize>) -> Result<usize, evm::Error> {
		...
		if valid_jump_destinations.contains(&jump) && jump_u < U256::from(!0 as usize) { Ok(jump) } else { Err(evm::Error::BadJumpDestination { destination: jump }) }
	}
	...
	fn exec_stack_instruction(&self, instruction: Instruction, stack: &mut Stack<U256>) -> Result<(), evm::Error> {
		match instruction {
			...
			instructions::DIV => {
				let a = stack.pop_back();
				let b = stack.pop_back();
				stack.push(if !self.is_zero(&b) { a.overflowing_div(b).0 } else { U256::zero() });
			}
			instructions::MOD => {
				let a = stack.pop_back();
				let b = stack.pop_back();
