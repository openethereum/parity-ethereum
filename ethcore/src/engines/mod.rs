...
/// Requested block data: optional full block, optional receipts.
pub type RequestedBlockData<'a> = (Option<&'a [u8]>, Option<&'a [Receipt]>);

/// An iterator over all blocks in the current epoch.
pub type Ancestry = Iterator<Item=Header>;

/// Results of a query of whether an epoch change occurred at the given block.
#[derive(Debug, Clone, PartialEq)]
pub enum EpochChange<A> {
	/// Cannot determine until more data is passed.
	/// Request needed data for a series of blocks, to be passed to the given closure.
	Unsure(Vec<(H256, Needs)>, Box<Fn(Vec<RequestedBlockData>) -> EpochChange<A>>),
	/// No epoch change.
	No,
	/// Validation proof required, activation kind, the new epoch number, and expected proof.
	Yes(u64, A, Bytes),
}

/// Activation of an epoch change: either instant or upon finality.
/// Hard-coded transitions in an validator multi-set take effect instantly,
/// while something like a contract changing validator set will require finality.
///
/// If an instant transition occurs on the same block that a finality-requiring change
/// occurs, the instant transition takes priority.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Activation {
	/// Activate instantly.
	Instant,
	/// Activate upon the block reaching finality.
	UponFinality,
}
...
pub trait Engine: Sync + Send {
	...
	fn is_epoch_end(&self, _header: &Header, RequestedBlockData, Ancestry)
		-> EpochChange<()>
	{
		EpochChange::No
	}
	...
	/// The implementation of this function should not draw on data other than the proof.
