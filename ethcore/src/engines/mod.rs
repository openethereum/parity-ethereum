...
use self::epoch::{Transition, PendingTransitionStore};
...
pub trait Engine : Sync + Send {
	...
	/// and it should always be equivalent to the proof of the last transition block.
	...
	///
	/// Return `Some` with optional finality proof if this block ends the epoch.
	/// `Some(Some(proof)) indicates epoch change with finality proof.
	/// `Some(None)` indicates no finality necessary.
	/// `None` indicates no epoch change.
	fn is_epoch_end(
		&self,
		_chain_head: &Header,
		_chain: &Headers,
	...
	fn epoch_verifier(&self, _header: &Header, _proof: &[u8]) -> Result<(Box<EpochVerifier>, bool), Error> {
		Ok((Box::new(self::epoch_verifier::NoOp), false))
	}
