...
mod tests {
	...
	fn fetches_validators() {
		let client = generate_dummy_client_with_spec(bundle::new_validator_safe_contract);
	...
	fn knows_validators() {
		...
		let chain_id = bundle::new_validator_safe_contract().chain_id();
		let client = generate_dummy_client_with_spec(bundle::new_validator_safe_contract);
		...
		// Check syncing.
		let sync_client = generate_dummy_client_with_spec_and_data(bundle::new_validator_safe_contract, 0, 0, &[]);
