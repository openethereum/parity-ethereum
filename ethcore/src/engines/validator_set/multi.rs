...
mod tests {
	...
	fn uses_current_set() {
		...
		let client = generate_dummy_client_with_spec(bundle::new_validator_multi);
		...
		// Check syncing.
		let sync_client = generate_dummy_client_with_spec_and_data(bundle::new_validator_multi, 0, 0, &[]);
