...
impl BlockChain {
	...
	pub fn insert_block(&self, bytes: &[u8], receipts: Vec<Receipt>) -> ImportRoute {
		...
		if let BlockLocation::BranchBecomingCanonChain(ref d) = info.location {
			...
			info!(target: "reorg", "{} {:?}", Colour::Red.bold().paint("Retracting"), d.retracted.iter().map(|h| format!("{}", h)).collect::<Vec<_>>());
			...
			info!(target: "reorg", "{} {:?}", Colour::Green.bold().paint("Enacting"), d.enacted.iter().map(|h| format!("{}", h)).collect::<Vec<_>>());
		}
