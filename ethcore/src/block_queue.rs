...
impl QueueSignal {
	...
	fn set(&self) {
		...
		if self.signalled.compare_and_swap(false, true, AtomicOrdering::Relaxed) == false {
			if let Err(e) = self.message_channel.send(SyncMessage::BlockVerified) {
...
impl BlockQueue {
	...
	pub fn new(config: BlockQueueConfig, engine: Arc<Box<Engine>>, message_channel: IoChannel<SyncMessage>) -> BlockQueue {
