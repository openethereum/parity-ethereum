...
fn restored_is_equivalent() {
	...
	let client = generate_dummy_client_with_spec_and_data(bundle::new_null, NUM_BLOCKS, TX_PER, &gas_prices);
...
fn guards_delete_folders() {
	let gas_prices = vec![1.into(), 2.into(), 3.into(), 999.into()];
	let client = generate_dummy_client_with_spec_and_data(bundle::new_null, 400, 5, &gas_prices);
...
fn recover_aborted_recovery() {
	...
	const NUM_BLOCKS: u32 = 400;
	let gas_prices = vec![1.into(), 2.into(), 3.into(), 999.into()];
	let client = generate_dummy_client_with_spec_and_data(bundle::new_null, NUM_BLOCKS, 5, &gas_prices);
