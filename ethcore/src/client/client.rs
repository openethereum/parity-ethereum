    ...
    where V: Verifier,
{
	...
	pub fn new_with_verifier(config: ClientConfig,
	                         spec: Spec,
	                         path: &Path,
	                         message_channel: IoChannel<NetSyncMessage>)
	                         -> Result<Arc<Client<V>>, Error> {
	...
	pub fn state(&self) -> State {
		State::from_existing(self.state_db.lock().unwrap().spawn(),
	...
	fn logs(&self, filter: Filter) -> Vec<LocalizedLogEntry> {
		...
		blocks.into_iter()
		      ...
		      .filter_map(|(number, hash, receipts)| {
			      self.chain.block(&hash).map(|ref b| (number, hash, receipts, BlockView::new(b).transaction_hashes()))
			     })
