...
impl Operations {
	...
	/// Auto-generated from: `{"constant":true,"inputs":[{"name":"_client","type":"bytes32"},{"name":"_release","type":"bytes32"}],"name":"findRelease","outputs":[{"name":"o_forkBlock","type":"uint32"},{"name":"o_track","type":"uint8"},{"name":"o_semver","type":"uint24"}],"type":"function"}`
	#[allow(dead_code)]
	pub fn find_release(&self, _client: &str, _release: &util::H256) -> Result<(u32, u8, u32), String> { 
		let call = self.contract.function("findRelease".into()).map_err(Self::as_string)?;
		let data = call.encode_call(
			vec![ethabi::Token::FixedBytes(_client.as_bytes().to_owned()), ethabi::Token::FixedBytes(_release.as_ref().to_owned())]
		).map_err(Self::as_string)?;
		let output = (self.do_call)(self.address.clone(), data)?;
		let returned = call.decode_output(output).map_err(Self::as_string)?;
		let mut result = returned.into_iter().rev().collect::<Vec<_>>();
	...
	/// Auto-generated from: `{"constant":true,"inputs":[{"name":"_client","type":"bytes32"},{"name":"_checksum","type":"bytes32"}],"name":"findBuild","outputs":[{"name":"o_release","type":"bytes32"},{"name":"o_platform","type":"bytes32"}],"type":"function"}`
	#[allow(dead_code)]
	pub fn find_build(&self, _client: &str, _checksum: &util::H256) -> Result<(util::H256, util::H256), String> { 
		let call = self.contract.function("findBuild".into()).map_err(Self::as_string)?;
		let data = call.encode_call(
	...
	pub fn propose_fork(&self, _number: u32, _name: &util::H256, _spec: &util::H256) -> Result<(), String> { 
		...
		let data = call.encode_call(
			vec![ethabi::Token::Uint({ let mut r = [0u8; 32]; util::U256::from(_number as u64).to_big_endian(&mut r); r }), ethabi::Token::FixedBytes(_name.as_ref().to_owned()), ethabi::Token::FixedBytes(_spec.as_ref().to_owned())]
		).map_err(Self::as_string)?;
		call.decode_output((self.do_call)(self.address.clone(), data)?).map_err(Self::as_string)?;
	...
	/// Auto-generated from: `{"constant":true,"inputs":[{"name":"_client","type":"bytes32"},{"name":"_release","type":"bytes32"},{"name":"_platform","type":"bytes32"}],"name":"findChecksum","outputs":[{"name":"","type":"bytes32"}],"type":"function"}`
	#[allow(dead_code)]
	pub fn find_checksum(&self, _client: &str, _release: &util::H256, _platform: &str) -> Result<util::H256, String> { 
