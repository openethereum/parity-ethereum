...
pub struct EthashParams {
	...
	/// Pause Difficulty Bomb growth after block
	pub bomb_pause_transition: u64,
	/// Continue Difficulty Bomb growth after block
	pub bomb_continue_transition: u64
}
...
impl From<ethjson::spec::EthashParams> for EthashParams {
	fn from(p: ethjson::spec::EthashParams) -> Self {
		EthashParams {
			...
			bomb_pause_transition: p.bomb_pause_transition.map_or(0x7fffffffffffffff, Into::into),
			bomb_continue_transition: p.bomb_continue_transition.map_or(0x7fffffffffffffff, Into::into),
		}
	}
}
...
impl Ethash {
	fn calculate_difficulty(&self, header: &Header, parent: &Header) -> U256 {
		...
		if header.number() < self.ethash_params.bomb_defuse_transition {
			if header.number() < self.ethash_params.bomb_pause_transition {
				let period = ((parent.number() + 1) / EXP_DIFF_PERIOD) as usize;
				if period > 1 {
					target = max(min_difficulty, target + (U256::from(1) << (period - 2)));
				}
			}
			else if header.number() < self.ethash_params.bomb_continue_transition {
				let fixed_difficulty = ((self.ethash_params.bomb_pause_transition / EXP_DIFF_PERIOD) - 2) as usize;
				target = max(min_difficulty, target + (U256::from(1) << fixed_difficulty));
			}
			else {
				let period = ((parent.number() + 1) / EXP_DIFF_PERIOD) as usize;
				let delay = ((self.ethash_params.bomb_continue_transition - self.ethash_params.bomb_pause_transition) / EXP_DIFF_PERIOD) as usize;
...
mod tests {
	...
	fn test_difficulty_bomb_continue() {
		...
		let ethparams = EthashParams {
			bomb_pause_transition: 3000000,
			bomb_continue_transition: 5000000,
			.. get_default_ethash_params()
		};
		let ethash = Ethash::new(spec.params, ethparams, BTreeMap::new());
