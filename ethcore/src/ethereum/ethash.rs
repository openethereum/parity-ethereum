...
impl Engine for Arc<Ethash> {
	...
	fn schedule(&self, block_number: BlockNumber) -> Schedule {
		...
		if block_number < self.ethash_params.homestead_transition {
			...
			/// There's no max_code_size transition so we tie it to eip150
			let max_code_size = if block_number >= self.ethash_params.eip150_transition { self.ethash_params.max_code_size as usize } else { usize::max_value() };
