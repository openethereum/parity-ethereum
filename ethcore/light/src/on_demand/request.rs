...
use bytes::Bytes;
use common_types::{
    basic_account::BasicAccount, encoded, receipt::Receipt, transaction::SignedTransaction,
};
use ethcore::{
    engines::{EthEngine, StateDependentProof},
    machine::EthereumMachine,
    state::{self, ProvedExecution},
};
use ethereum_types::{Address, H256, U256};
use ethtrie::{TrieDB, TrieError};
use hash::{keccak, KECCAK_EMPTY, KECCAK_EMPTY_LIST_RLP, KECCAK_NULL_RLP};
use hash_db::HashDB;
use kvdb::DBValue;
use parking_lot::Mutex;
use request::{self as net_request, CompleteRequest, Field, IncompleteRequest, Output, OutputKind};
use rlp::{Rlp, RlpStream};
use trie::Trie;
use vm::EnvInfo;

const SUPPLIED_MATCHES: &str =
    "supplied responses always match produced requests; enforced by `check_response`; qed";
...
impl_single!(
    TransactionIndex,
    TransactionIndex,
    net_request::TransactionIndexResponse
);
...
mod impls {
    ...
    use super::{Request, RequestAdapter, RequestArg, Response, SUPPLIED_MATCHES};
...
pub enum CheckedRequest {
    ...
    TransactionIndex(
        TransactionIndex,
        net_request::IncompleteTransactionIndexRequest,
    ),
    ...
    fn from(req: Request) -> Self {
        match req {
            ...
            Request::TransactionIndex(req) => {
                let net_req = net_request::IncompleteTransactionIndexRequest { hash: req.0 };
        ...
        match *self {
            ...
            CheckedRequest::Receipts(ref check, ref req) => {
                ...
                if check
                    ...
                    .as_ref()
                    .ok()
                    .map_or(false, |hdr| hdr.receipts_root() == KECCAK_NULL_RLP)
                {
            ...
            CheckedRequest::Body(ref check, ref req) => {
                ...
                if let Ok(hdr) = check.0.as_ref() {
                    if hdr.transactions_root() == KECCAK_NULL_RLP
                        && hdr.uncles_hash() == KECCAK_EMPTY_LIST_RLP
                    {
                ...
                block_header
                    ...
                    .map(|(hdr, body)| {
                        Response::Body(encoded::Block::new_from_header_and_body(
                            &hdr.view(),
                            &body.view(),
                        ))
                    })
...
impl IncompleteRequest for CheckedRequest {
    ...
    where
        F: FnMut(usize, usize, OutputKind) -> Result<(), net_request::NoSuchOutput>,
    {
        match *self {
    ...
    fn fill<F>(&mut self, f: F)
    where
        F: Fn(usize, usize) -> Result<Output, net_request::NoSuchOutput>,
    {
...
impl net_request::CheckedRequest for CheckedRequest {
    ...
    fn check_response(
        ...
        match *self {
            ...
            CheckedRequest::Body(ref prover, _) => expect!((&NetResponse::Body(ref res), _) =>
            ...
            CheckedRequest::Signal(ref prover, _) => expect!((&NetResponse::Signal(ref res), _) =>
...
impl HeaderProof {
    ...
    pub fn cht_num(&self) -> u64 {
        self.cht_num
    }

    /// Access the expected CHT root.
    pub fn cht_root(&self) -> H256 {
        self.cht_root
    }
...
impl HeaderWithAncestors {
    ...
    pub fn check_response(
        ...
        match actual_len.cmp(&expected_len) {
            cmp::Ordering::Less => return Err(Error::TooFewResults(expected_len, actual_len)),
            cmp::Ordering::Greater => return Err(Error::TooManyResults(expected_len, actual_len)),
            cmp::Ordering::Equal => (),
        };
...
impl Body {
    ...
    pub fn check_response(
        ...
        cache: &Mutex<::cache::Cache>,
        body: &encoded::Body,
    ) -> Result<encoded::Block, Error> {
        // check the integrity of the the body against the header
        let header = self.0.as_ref()?;
        let tx_root =
            ::triehash::ordered_trie_root(body.transactions_rlp().iter().map(|r| r.as_raw()));
...
impl BlockReceipts {
    ...
    pub fn check_response(
        ...
        if receipts_root == found_root {
            cache
                .lock()
                .insert_block_receipts(receipts_root, receipts.to_vec());
            Ok(receipts.to_vec())
        } else {
...
impl Account {
    ...
    pub fn check_response(
        ...
        _: &Mutex<::cache::Cache>,
        proof: &[Bytes],
    ) -> Result<Option<BasicAccount>, Error> {
...
impl TransactionProof {
    ...
    pub fn check_response(
        ...
        _: &Mutex<::cache::Cache>,
        state_items: &[DBValue],
    ) -> Result<super::ExecutionResult, Error> {
...
impl Signal {
    ...
    pub fn check_response(
        ...
        self.proof_check
            .check_proof(self.engine.machine(), signal)
...
mod tests {
    ...
    use ethcore::client::{BlockChainClient, BlockInfo, EachBlockWith, TestBlockChainClient};
    ...
    fn check_header_by_hash() {
        ...
        let cache = Mutex::new(make_cache());
        assert!(HeaderByHash(hash.into())
            .check_response(&cache, &hash.into(), &[raw_header])
            .is_ok())
    }
    ...
    fn check_header_with_ancestors() {
        ...
        let header_with_ancestors = |hash, count| HeaderWithAncestors {
            block_hash: hash,
        ...
        assert!(header_with_ancestors(headers[0].hash().into(), 2)
            .check_response(&cache, &headers[0].hash().into(), &raw_headers[0..3])
            .is_ok());
        ...
        assert!(header_with_ancestors(headers[2].hash().into(), 2)
            .check_response(&cache, &headers[2].hash().into(), &raw_headers[2..5])
            .is_ok());
        assert!(header_with_ancestors(headers[2].hash().into(), 10)
            .check_response(&cache, &headers[2].hash().into(), &raw_headers[2..11])
            .is_ok());
        assert!(header_with_ancestors(invalid_successor.hash().into(), 0)
            .check_response(
                &cache,
                &invalid_successor.hash().into(),
                &[raw_invalid_successor.clone()]
            )
            .is_ok());

        // Incorrect responses
        assert_eq!(
            header_with_ancestors(invalid_successor.hash().into(), 0).check_response(
                &cache,
                &headers[0].hash().into(),
                &raw_headers[0..1]
            ),
            Err(Error::WrongHash(
                invalid_successor.hash(),
                headers[0].hash()
            ))
        );
        assert_eq!(
            header_with_ancestors(headers[0].hash().into(), 0).check_response(
                &cache,
                &headers[0].hash().into(),
                &[]
            ),
        ...
        assert_eq!(
            header_with_ancestors(headers[0].hash().into(), 10).check_response(
                &cache,
                &headers[0].hash().into(),
                &raw_headers[0..10]
            ),
        ...
        assert_eq!(
            header_with_ancestors(headers[0].hash().into(), 9).check_response(
                &cache,
                &headers[0].hash().into(),
                &raw_headers[0..11]
            ),
        ...
        assert_eq!(
            header_with_ancestors(headers[0].hash().into(), 1).check_response(
                &cache,
                &headers[0].hash().into(),
                response
            ),
        ...
        assert_eq!(
            header_with_ancestors(invalid_successor.hash().into(), 1).check_response(
                &cache,
                &invalid_successor.hash().into(),
                response
            ),
    ...
    fn check_receipts() {
        ...
        let mut header = Header::new();
        let receipts_root =
            ::triehash::ordered_trie_root(receipts.iter().map(|x| ::rlp::encode(x)));
