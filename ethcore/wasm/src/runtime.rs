...
impl<'a> Runtime<'a> {
	...
	fn do_create(&mut self, endowment: U256, code_ptr: u32, code_len: u32, result_ptr: u32, scheme: vm::CreateContractAddress) -> Result<RuntimeValue> {
		...
		let create_type = match scheme {
			vm::CreateContractAddress::FromSenderAndNonce => CallType::Create,
			vm::CreateContractAddress::FromSenderSaltAndCodeHash(_) => CallType::Create2,
			vm::CreateContractAddress::FromSenderAndCodeHash => CallType::Create,
		};
