...
impl Provider where {
	...
	fn get_decrypted_state(&self, address: &Address, block: BlockId) -> Result<Bytes, PrivateTransactionError> {
		...
		let state = contract.state(|addr, data| futures::done(self.client.call_contract(block, addr, data))).wait()
	...
	fn get_decrypted_code(&self, address: &Address, block: BlockId) -> Result<Bytes, PrivateTransactionError> {
		let contract = Contract::new(*address);
		let code = contract.code(|addr, data| futures::done(self.client.call_contract(block, addr, data))).wait()
			.map_err(|e| PrivateTransactionError::Call(e))?;
		self.decrypt(address, &code)
	}

	fn get_contract_nonce(&self, address: &Address, block: BlockId) -> Result<U256, PrivateTransactionError> {
		let contract = Contract::new(*address);
		let nonce = contract.nonce(|addr, data| futures::done(self.client.call_contract(block, addr, data))).wait()
			.map_err(|e| PrivateTransactionError::Call(e))?;
		Ok(nonce)
	}
