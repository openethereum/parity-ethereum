...
use common_types::{filter::Filter, BlockNumber};
...
impl OperationsContractClient {
    ...
    {
        let (data, decoder) =
            operations::functions::latest_in_track::call(*CLIENT_ID_HASH, u8::from(track));
        ...
        F: Fn(Vec<u8>) -> Result<Vec<u8>, String>,
    {
        ...
        let (fork, track, semver, is_critical) =
            decoder.decode(&do_call(data)?).map_err(|e| e.to_string())?;

        let (fork, track, semver) = (fork.low_u64(), track.low_u32(), semver.low_u32());

        let (data, decoder) =
            operations::functions::checksum::call(*CLIENT_ID_HASH, release_id, *PLATFORM_ID_HASH);
...
impl OperationsClient for OperationsContractClient {
    fn latest(&self, this: &VersionInfo, track: ReleaseTrack) -> Result<OperationsInfo, String> {
        ...
        const PROOF: &str =
            "in_minor initialized and assigned with Some; loop breaks if None assigned; qed";
    ...
    fn release_block_number(
        ...
        let topics = operations::events::release_added::filter(
            Some(*CLIENT_ID_HASH),
        ...
        client
            ...
            .iter()
            .filter_map(|log| {
                let event = operations::events::release_added::parse_log(
...
impl Updater {
    ...
    pub fn new(
        ...
        let r = Arc::new(Updater {
            ...
            fetcher,
            operations_client: OperationsContractClient::new(client.clone()),
