...
pub fn execute<D: Dispatcher + 'static>(
	...
	match payload {
		...
		ConfirmationPayload::PostSignTransaction(address, mut data) => {
	...
	where D: Dispatcher
{
	match payload {
		...
		RpcConfirmationPayload::PostSignTransaction(RpcSignRequest { address, data }) => {
			future::ok(ConfirmationPayload::PostSignTransaction(address.into(), data.into())).boxed()
		},
	}
}
