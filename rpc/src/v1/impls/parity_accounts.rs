...
impl ParityAccounts for ParityAccountsClient {
	...
	fn derive_key_index(&self, addr: RpcH160, password: String, derivation: DerivateHierarchical, save_as_account: bool) -> Result<RpcH160, Error> {
		let addr: Address = addr.into();
		take_weak!(self.accounts)
			.derive_account(
				&addr,
				Some(password),
				Derivate::from(derivation).to_derivation()
	...
	fn derive_key_hash(&self, addr: RpcH160, password: String, derivation: DerivateHash, save_as_account: bool) -> Result<RpcH160, Error> {
		let addr: Address = addr.into();
		take_weak!(self.accounts)
			.derive_account(
				&addr,
