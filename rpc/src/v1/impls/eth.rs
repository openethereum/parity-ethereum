...
use v1::types::{Block, BlockNumber, BlockTransactions, Bytes, CallRequest, Filter, Index, Log, OptionalValue, Receipt, SyncInfo, SyncStatus, Transaction, TransactionRequest};
		...
		match (client.block(id.clone()), client.block_total_difficulty(id)) {
			(Some(bytes), Some(total_difficulty)) => {
				...
				let block = Block {
					...
					transactions: {
						if include_txs { BlockTransactions::Full(block_view.localized_transactions().into_iter().map(From::from).collect()) } else { BlockTransactions::Hashes(block_view.transaction_hashes()) }
					},
...
fn from_params_discard_second<F>(params: Params) -> Result<(F,), Error>
	where F: serde::de::Deserialize,
{
...
fn from_params_discard_third<F1, F2>(params: Params) -> Result<(F1, F2), Error>
	where F1: serde::de::Deserialize,
	      F2: serde::de::Deserialize,
{
	match params_len(&params) {
	      ...
	      EM: ExternalMinerService + 'static,
{
	...
	fn balance(&self, params: Params) -> Result<Value, Error> {
		from_params_discard_second(params).and_then(|(address,)| to_value(&take_weak!(self.client).balance(&address)))
	}

	fn storage_at(&self, params: Params) -> Result<Value, Error> {
		from_params_discard_third::<Address, U256>(params).and_then(|(address, position)| to_value(&U256::from(take_weak!(self.client).storage_at(&address, &H256::from(position)))))
	}

	fn transaction_count(&self, params: Params) -> Result<Value, Error> {
		from_params_discard_second(params).and_then(|(address,)| to_value(&take_weak!(self.client).nonce(&address)))
	}
	...
	fn code_at(&self, params: Params) -> Result<Value, Error> {
		from_params_discard_second(params).and_then(|(address,)| to_value(&take_weak!(self.client).code(&address).map_or_else(Bytes::default, Bytes::new)))
	}
	...
	fn transaction_by_block_number_and_index(&self, params: Params) -> Result<Value, Error> {
		from_params::<(BlockNumber, Index)>(params).and_then(|(number, index)| self.transaction(TransactionId::Location(number.into(), index.value())))
	}
