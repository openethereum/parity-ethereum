...
#[test]
fn rpc_parity_pending_transactions_filtered() {
	let deps = Dependencies::new();
	let io = deps.default_client();
	let tx1 = ::types::transaction::Transaction {
		value: 5.into(),
		gas: 3.into(),
		gas_price: 2.into(),
		action: ::types::transaction::Action::Call(Address::from(1_000)), // same receiver as tx2
		data: vec![1, 2, 3],
		nonce: 0.into(),
	}.fake_sign(10.into()); // same sender as tx3
	let content_tx1 = r#"
		{
			"blockHash": null,
			"blockNumber": null,
			...
			"gas": "0x3",
			"gasPrice": "0x2",
			"hash": "0xb8d44001a15402b927e6f1f397f0d61d67e560f0376ef01c13d04b0f0a87adb8",
			"input": "0x010203",
			"nonce": "0x0",
			"publicKey": null,
			"r": "0x1",
			"raw": "0xe08002039400000000000000000000000000000000000003e80583010203800101",
			"s": "0x1",
			"standardV": "0x4",
			"to": "0x00000000000000000000000000000000000003e8",
	...
	let tx2 = ::types::transaction::Transaction {
		value: 5.into(),
		gas: 3.into(),
		gas_price: 2.into(),
		action: ::types::transaction::Action::Call(Address::from(1_000)), // same receiver as tx1
		data: vec![1, 2, 3],
		nonce: 0.into(),
	}.fake_sign(20.into());
	let content_tx2 = r#"
	...
	let tx3 = ::types::transaction::Transaction {
		value: 5.into(),
		gas: 3.into(),
		gas_price: 2.into(),
		action: ::types::transaction::Action::Call(Address::from(2_000)),
		data: vec![1, 2, 3],
		nonce: 0.into(),
	}.fake_sign(10.into()); // same sender as tx1
	let content_tx3 = r#"

	"#;

	//deps.miner.pending_transactions.lock().insert(H256::from(100), tx1);
	deps.miner.pending_transactions.lock().insert(H256::from(0), tx2);
	//deps.miner.pending_transactions.lock().insert(H256::from(300), tx3);

	let request = r#"{"jsonrpc": "2.0", "method": "parity_pendingTransactions", "params":[], "id": 1}"#;

	println!("RES: {:?}", io.handle_request_sync(request));
	println!("\n\n\n");

	//assert_eq!(io.handle_request_sync(request), Some(response_all.to_owned()));
}
