...
fn should_return_list_of_items_to_confirm() {
	...
	tester.signer.add_request(ConfirmationPayload::PostSignTransaction(1.into(), vec![5].into()), Origin::Unknown).unwrap();
...
fn should_not_remove_sign_if_password_is_invalid() {
	// given
	let tester = signer_tester();
	tester.signer.add_request(ConfirmationPayload::PostSignTransaction(0.into(), vec![5].into()), Origin::Unknown).unwrap();
	assert_eq!(tester.signer.requests().len(), 1);
