...
use txpool::{self, scoring::{Choice, Scoring}, Transaction, Readiness, ReplaceTransaction};
...
impl<S, C> txpool::ShouldReplace<VerifiedTransaction> for NonceAndGasPriceAndReadiness<S, C>
	...
	fn should_replace(
		&mut self,
		old_replace: txpool::ReplaceTransaction<VerifiedTransaction>,
		new_replace: txpool::ReplaceTransaction<VerifiedTransaction>,
	) -> Choice {
		let old = old_replace.transaction;
		let new = new_replace.transaction;
		...
		if old.sender() == new.sender() {
			...
			if new_score > old_score {
				...
				let is_ready = |replace: ReplaceTransaction<VerifiedTransaction>| {
					let mut state_nonce = state.account_nonce(replace.transaction.sender());
					let nonce =
				...
				if !is_ready(new_replace) && is_ready(old_replace) {
...
mod tests {
	...
	fn should_always_accept_local_transactions_unless_same_sender_and_nonce() {
		...
		assert_eq!(replace.should_replace(&same_sender_tx1, &same_sender_tx2, None), InsertNew);
		assert_eq!(replace.should_replace(&same_sender_tx2, &same_sender_tx1, None), InsertNew);

		assert_eq!(replace.should_replace(&different_sender_tx1, &different_sender_tx2, None), InsertNew);
		assert_eq!(replace.should_replace(&different_sender_tx2, &different_sender_tx1, None), InsertNew);

		// txs with same sender and nonce
		assert_eq!(replace.should_replace(&same_sender_tx2, &same_sender_tx3, None), ReplaceOld);
		assert_eq!(replace.should_replace(&same_sender_tx3, &same_sender_tx2, None), RejectNew);
	}
	...
	fn should_replace_same_sender_by_nonce() {
		...
		assert_eq!(replace.should_replace(&txs[0], &txs[1], None), RejectNew);
		assert_eq!(replace.should_replace(&txs[1], &txs[0], None), ReplaceOld);

		assert_eq!(replace.should_replace(&txs[1], &txs[2], None), RejectNew);
		assert_eq!(replace.should_replace(&txs[2], &txs[1], None), RejectNew);

		assert_eq!(replace.should_replace(&txs[1], &txs[3], None), ReplaceOld);
		assert_eq!(replace.should_replace(&txs[3], &txs[1], None), RejectNew);
	}
	...
	fn should_replace_different_sender_by_priority_and_gas_price() {
		...
		assert_eq!(replace.should_replace(&tx_regular_low_gas, &tx_regular_high_gas, None), ReplaceOld);
		assert_eq!(replace.should_replace(&tx_regular_high_gas, &tx_regular_low_gas, None), RejectNew);

		assert_eq!(replace.should_replace(&tx_regular_high_gas, &tx_local_low_gas, None), ReplaceOld);
		assert_eq!(replace.should_replace(&tx_local_low_gas, &tx_regular_high_gas, None), RejectNew);

		assert_eq!(replace.should_replace(&tx_local_low_gas, &tx_local_high_gas, None), InsertNew);
		assert_eq!(replace.should_replace(&tx_local_high_gas, &tx_regular_low_gas, None), RejectNew);
	}
}
