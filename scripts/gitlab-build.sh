...
strip_binaries () {
  echo "Strip binaries:"
  $STRIP_BIN -v target/$PLATFORM/release/purity
...
calculate_checksums () {
  ...
  BIN="target/$PLATFORM/release/purity$S3WIN"
  export SHA3="$($BIN tools hash $BIN)"

  echo "Purity file SHA3: $SHA3"
  $MD5_BIN target/$PLATFORM/release/purity$S3WIN > purity$S3WIN.md5
  $SHA256_BIN target/$PLATFORM/release/purity$S3WIN > purity$S3WIN.sha256
...
make_deb () {
  ...
  cp target/$PLATFORM/release/purity deb/usr/bin/purity
  ...
  dpkg-deb -b deb "purity_"$VER"_"$IDENT"_"$ARC".deb"
  $MD5_BIN "purity_"$VER"_"$IDENT"_"$ARC".deb" > "purity_"$VER"_"$IDENT"_"$ARC".deb.md5"
  $SHA256_BIN "purity_"$VER"_"$IDENT"_"$ARC".deb" > "purity_"$VER"_"$IDENT"_"$ARC".deb.sha256"
}
make_rpm () {
  rm -rf /install
  mkdir -p /install/usr/bin
  cp target/$PLATFORM/release/purity /install/usr/bin
  ...
  rm -rf "purity-"$VER"-1."$ARC".rpm" || true
  fpm -s dir -t rpm -n purity -v $VER --epoch 1 --license GPLv3 -d openssl --provides purity --url https://parity.io --vendor "Parity Technologies" -a x86_64 -m "<devops@parity.io>" --description "Ethereum network client by Parity Technologies" -C /install/
  cp "purity-"$VER"-1."$ARC".rpm" "purity_"$VER"_"$IDENT"_"$ARC".rpm"
  $MD5_BIN "purity_"$VER"_"$IDENT"_"$ARC".rpm" > "purity_"$VER"_"$IDENT"_"$ARC".rpm.md5"
  $SHA256_BIN "purity_"$VER"_"$IDENT"_"$ARC".rpm" > "purity_"$VER"_"$IDENT"_"$ARC".rpm.sha256"
}
make_pkg () {
  echo "make PKG"
  cp target/$PLATFORM/release/purity target/release/purity
  ...
  mv target/release/Parity\ Ethereum-signed.pkg "purity_"$VER"_"$IDENT"_"$ARC".pkg"
  $MD5_BIN "purity_"$VER"_"$IDENT"_"$ARC"."$EXT >> "purity_"$VER"_"$IDENT"_"$ARC".pkg.md5"
  $SHA256_BIN "purity_"$VER"_"$IDENT"_"$ARC"."$EXT >> "purity_"$VER"_"$IDENT"_"$ARC".pkg.sha256"
}
sign_exe () {
  ./sign.cmd $keyfile $certpass "target/$PLATFORM/release/purity.exe"
}
make_exe () {
  ./msbuild.cmd
  ...
  ./nsis.cmd
  cd ..
  cp nsis/installer.exe "purity_"$VER"_"$IDENT"_"$ARC"."$EXT
  ./sign.cmd $keyfile $certpass "purity_"$VER"_"$IDENT"_"$ARC"."$EXT
  $MD5_BIN "purity_"$VER"_"$IDENT"_"$ARC"."$EXT -p %h > "purity_"$VER"_"$IDENT"_"$ARC"."$EXT".md5"
  $SHA256_BIN "purity_"$VER"_"$IDENT"_"$ARC"."$EXT -p %h > "purity_"$VER"_"$IDENT"_"$ARC"."$EXT".sha256"
}
push_binaries () {
  ...
  aws s3api put-object --bucket $S3_BUCKET --key $CI_BUILD_REF_NAME/$BUILD_PLATFORM/purity$S3WIN --body target/$PLATFORM/release/purity$S3WIN
  aws s3api put-object --bucket $S3_BUCKET --key $CI_BUILD_REF_NAME/$BUILD_PLATFORM/purity$S3WIN.md5 --body purity$S3WIN.md5
  aws s3api put-object --bucket $S3_BUCKET --key $CI_BUILD_REF_NAME/$BUILD_PLATFORM/purity$S3WIN.sha256 --body purity$S3WIN.sha256
  ...
  aws s3api put-object --bucket $S3_BUCKET --key $CI_BUILD_REF_NAME/$BUILD_PLATFORM/"purity_"$VER"_"$IDENT"_"$ARC"."$EXT --body "purity_"$VER"_"$IDENT"_"$ARC"."$EXT
  aws s3api put-object --bucket $S3_BUCKET --key $CI_BUILD_REF_NAME/$BUILD_PLATFORM/"purity_"$VER"_"$IDENT"_"$ARC"."$EXT".md5" --body "purity_"$VER"_"$IDENT"_"$ARC"."$EXT".md5"
  aws s3api put-object --bucket $S3_BUCKET --key $CI_BUILD_REF_NAME/$BUILD_PLATFORM/"purity_"$VER"_"$IDENT"_"$ARC"."$EXT".sha256" --body "purity_"$VER"_"$IDENT"_"$ARC"."$EXT".sha256"
}
make_archive () {
  echo "add artifacts to archive"
  rm -rf parity.zip
  zip -r parity.zip target/$PLATFORM/release/purity$S3WIN target/$PLATFORM/release/parity-evm$S3WIN target/$PLATFORM/release/ethstore$S3WIN target/$PLATFORM/release/ethkey$S3WIN target/$PLATFORM/release/whisper$S3WIN purity$S3WIN.md5 parity-evm$S3WIN.md5 ethstore$S3WIN.md5 ethkey$S3WIN.md5 whisper$S3WIN.md5 purity$S3WIN.sha256 parity-evm$S3WIN.sha256 ethstore$S3WIN.sha256 ethkey$S3WIN.sha256 whisper$S3WIN.sha256
}

updater_push_release () {
  echo "push release"

  DATA="commit=$CI_BUILD_REF&sha3=$SHA3&filename=purity$S3WIN&secret=$RELEASES_SECRET"
  # Mainnet
