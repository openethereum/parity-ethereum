...
function convertContract(name, json, prefs) {
	...
	${prefs._explicit_do_call ? "" : `do_call: Box<Fn(${prefs._custom_block ? "BlockId, " : ""}util::Address, Vec<u8>) -> Result<Vec<u8>, String> + Send${prefs._sync ? " + Sync " : ""}+ 'static>,`}
}
impl ${name} {
	pub fn new${prefs._explicit_do_call ? "" : "<F>"}(address: util::Address${prefs._explicit_do_call ? "" : `, do_call: F`}) -> Self
		${prefs._explicit_do_call ? "" : `where F: Fn(${prefs._custom_block ? "BlockId, " : ""}util::Address, Vec<u8>) -> Result<Vec<u8>, String> + Send ${prefs._sync ? "+ Sync " : ""}+ 'static`} {
		${name} {
...
function convertFunction(json, _prefs) {
	...
	#[allow(dead_code)]
	pub fn ${snakeName}${cprefs._explicit_do_call ? "<F>" : ""}(&self${cprefs._explicit_do_call ? `, do_call: &F` : ""}${cprefs._custom_block ? ", block_id: BlockId" : ''}${params.length > 0 ? ', ' + params.join(", ") : ''}) -> Result<${returns}, String>
		${cprefs._explicit_do_call ? `where F: Fn(${prefs._custom_block ? "BlockId, " : ""}, util::Address, Vec<u8>) -> Result<Vec<u8>, String> + Send ${prefs._sync ? "+ Sync " : ""}` : ""} {
		...
		${json.outputs.length > 0 ? 'let output = ' : ''}call.decode_output((${cprefs._explicit_do_call ? "" : "self."}do_call)(${cprefs._custom_block ? "block_id, " : ""}self.address.clone(), data)?).map_err(Self::as_string)?;
...
for (let i = 1; i < process.argv.length; ++i) {
	...
	} else if (arg.indexOf("--custom-block") == 0) {
		prefs._custom_block = true;
